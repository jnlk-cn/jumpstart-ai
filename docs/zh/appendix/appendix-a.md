...
| **快速上手** | ```python<br>from zhipuai import ZhipuAI<br>client = ZhipuAI(api_key="xxx")<n>response = client.chat.completions.create(model="glm-4", messages=[{"role": "user", "content": "你好"}])\nprint(response.choices[0].message.content)``` |
...
| **快速上手** | ```python<br>import chromadb\nclient = chromadb.Client()<n>collection = client.create_collection("documents")<n>collection.add(ids=["1"], embeddings=[[0.1, 0.2]], documents=["示例文档"])\nresults = collection.query(query_embeddings=[[0.1, 0.2]], n_results=1)``` |
...
| **快速上手** | ```python<br>from qdrant_client import QdrantClient\nclient = QdrantClient(url="http://localhost:6333")<n>client.upload_collection(collection_name="docs", vectors={"1": [0.1, 0.2]}, payload=[{"text": "文档内容"}])\nresults = client.search(collection_name="docs", query_vector=[0.1, 0.2], limit=5)``` |
...
| **快速上手** | ```python<br>import pinecone\npinecone.init(api_key="xxx", environment="us-west1-gcp")<n>pinecone.create_index("documents", dimension=1536)<n>index = pinecone.Index("documents")<n>index.upsert([("1", [0.1]*1536, {"text": "文档内容"})])\nresults = index.query(vector=[0.1]*1536, top_k=5)``` |
...
| **快速上手** | ```python<br>import weaviate\nclient = weaviate.Client("http://localhost:8080")<n>client.data_object.create(class_name="Document", properties={"content": "文档内容"})\nresults = client.query.get("Document", ["content"]).with_near_vector({"vector": [0.1, 0.2]}).do()``` |
